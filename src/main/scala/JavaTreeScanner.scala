import com.sun.source.tree.{CaseTree, CompilationUnitTree, ErroneousTree, IdentifierTree, ImportTree, MemberReferenceTree, MemberSelectTree, Tree}
import com.sun.source.util.{JavacTask, TreePath, TreePathScanner, Trees}

class JavaTreeScanner(task: JavacTask, var root: CompilationUnitTree) extends TreePathScanner[TreePath, Int] {
  override def visitCompilationUnit(t: CompilationUnitTree, find: Int): TreePath = {
    root = t
  ...
  override def visitMemberReference(node: MemberReferenceTree, p: Int): TreePath = {
    val pos = Trees.instance(task).getSourcePositions
  ...
  override def visitErroneous(node: ErroneousTree, p: Int): TreePath = scan(node.getErrorTrees, p)
